...
export function VentasPage() {
  ...
  const ventas = useMemo(() => documentos.filter((d) => d.clase === 'Venta'), [documentos])
  const ventasFiltradas = ventas.filter((d) => filtro === 'Todos' || d.estado === filtro)
  ...
  return (
    <div>
      <Topbar title="Ventas" description="Libro de ventas y emisión de documentos (DTE)" />
      ...
      <div className="space-y-6 p-8">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <Card className="p-5">
            <p className="text-xs font-medium text-slate-500">IVA débito fiscal</p>
        ...
        <Card>
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 p-5">
            ...
            <Button size="sm" onClick={openNuevoDocumento}>
              <Plus size={15} /> Emitir documento
            </Button>
          </div>
      ...
      <DocumentoModal open={modalOpen} onClose={closeModal} clase="Venta" documento={editingDoc} />
    </div>
  )
}
